...
function triggerFeedback(elementId) {
    const el = document.getElementById(elementId);
    if (!el) return;
    el.classList.remove('blink-effect');
...
fileInput.addEventListener('change', (e) => {
    ...
    files.forEach(file => {
        playlist.push({ name: file.name.replace(/\.[^/.]+$/, ""), url: URL.createObjectURL(file), file: file });
    });
    renderPlaylist();
...
function extractMetadata(file) {
    ...
    jsmediatags.read(file, {
        onSuccess: function(tag) {
            ...
            badge.innerText = ext; badge.style.display = 'inline-block';
            const art = t.picture;
            if (art) {
                let base64String = "";
...
playPauseBtn.onclick = () => {
    ...
    triggerFeedback('play-pause-btn');
...
function toggleRepeat() {
    ...
    else if (repeatMode === 'ONE') { repeatMode = 'ALL'; btn.innerText = 'REPEAT: ALL'; btn.classList.replace('rep-one', 'rep-all'); }
    else { repeatMode = 'OFF'; btn.innerText = 'REPEAT: OFF'; btn.classList.remove('rep-all'); }
}

function toggleMute() {
    player.muted = !player.muted;
    triggerFeedback('mute-btn');
